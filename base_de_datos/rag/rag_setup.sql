...
-- 7. Tabla para tracking de progreso por unidad (pasos individuales completados)
CREATE TABLE IF NOT EXISTS public.progreso_unidades (
    id SERIAL PRIMARY KEY,
    perfil_id UUID REFERENCES public.perfiles(id) ON DELETE CASCADE,
    step_id INTEGER REFERENCES public.learning_path_steps(id) ON DELETE CASCADE,
    curso_id INTEGER REFERENCES public.cursos(id) ON DELETE CASCADE,
    completado BOOLEAN DEFAULT FALSE,
    fecha_completado TIMESTAMP WITH TIME ZONE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(perfil_id, step_id)
);

ALTER TABLE public.progreso_unidades ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Usuarios pueden ver su propio progreso" 
ON public.progreso_unidades
FOR SELECT TO authenticated
