> ⚠️ **Documento desactualizado**: propone integrar vía rutas API de Next.js, pero la integración real usa `lib/api-service.ts` contra el backend FastAPI (`backend/`) más Supabase Auth directo, no rutas `/app/api/`. Revisar/reescribir antes de usarlo como referencia.
